...
joints = zeros(3,4); %columns: lhl,lfl,rhl,rfl
% Left hind leg


Mlh_lhp = [0,1,0,0;-1,0,0,sum(l);0,0,1,0;0,0,0,1]; 
...
Tlh_lhp = expm(S1_lh_box*joints(1))*expm(S2_lh_box*joints(2))...
    *expm(S3_lh_box*joints(3))*Mlh_lhp;
...
Tlhp_lh = Mlhp_lh*expm(B1_lhp_box*joints(3))*expm(B2_lhp_box*joints(2))...
    *expm(B3_lhp_box*joints(1));
...
Tls_lfp = expm(S1_ls_box*joints(4))*expm(S2_ls_box*joints(5))...
    *expm(S3_ls_box*joints(6))*Mls_lfp;
...
Tlfp_ls = Mlfp_ls*expm(B1_lfp_box*joints(6))*expm(B2_lfp_box*joints(5))...
    *expm(B3_lfp_box*joints(4));
...
Trh_rhp = expm(S1_rh_box*joints(7))*expm(S2_rh_box*joints(8))...
    *expm(S3_rh_box*joints(9))*Mrh_rhp;
...
Trhp_rh = Mrhp_rh*expm(B1_rhp_box*joints(9))*expm(B2_rhp_box*joints(8))...
    *expm(B3_rhp_box*joints(7));
...
Trs_rfp = expm(S1_rs_box*joints(10))*expm(S2_rs_box*joints(11))...
    *expm(S3_rs_box*joints(12))*Mrs_rfp;
...
Trfp_rs = expm(B1_rfp_box*joints(12))*expm(B2_rfp_box*joints(11))...
    *expm(S3_rs_box*joints(10))*Mrfp_rs;
